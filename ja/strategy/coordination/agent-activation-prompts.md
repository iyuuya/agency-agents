...
> NEXUSパイプライン内の任意のエージェントを起動するための即使用可能なプロンプトテンプレート。`[プレースホルダー]`をカスタマイズしてデプロイしてください。

---

## パイプラインコントローラー

### Agents Orchestrator — 完全パイプライン
```
...
## クイックリファレンス: どの状況でどのプロンプトを使うか

| 状況 | 主要プロンプト | サポートプロンプト |
|-----------|---------------|-----------------|
| 新しいプロジェクトを開始する | オーケストレーター — 完全パイプライン | — |
| 機能を構築する | オーケストレーター — Dev↔QAループ | 開発者 + Evidence Collector |
| バグを修正する | Backend/Frontend Developer | API Tester または Evidence Collector |
| キャンペーンを実施する | Content Creator | Social Media Strategist + プラットフォームエージェント |
| ローンチの準備をする | フェーズ5プレイブックを参照 | 全マーケティング + DevOpsエージェント |
| 月次レポート | Executive Summary Generator | Analytics Reporter + Finance Tracker |
| インシデント対応 | Infrastructure Maintainer | DevOps Automator + 関連する開発者 |
